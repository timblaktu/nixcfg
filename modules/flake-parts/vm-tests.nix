      ...
      checks = {
        ...
        vm-nspawn-smoke = mkContainerTest {
          ...
          testScript = ''
            ...
            # NSPAWN FINDING (POC): the QEMU twin vm-system-type-cli asserts
            # `wait_for_unit("sshd.service")` and passes, but in the nspawn container sshd.service
            # is inactive at boot (socket-activated / not-started under systemd-nspawn) — a real
            # VM-vs-container service-activation difference to watch when migrating ssh-dependent
            # tests. Assert sshd *setup* instead (host key generated by sshd-keygen), which is
            # activation-agnostic and still proves the cli layer configured openssh.
