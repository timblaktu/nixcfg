...
{ config, lib, inputs, ... }:
{
  flake.modules = {
    ...
    homeManager.neovim = { config, pkgs, lib, ... }:
      let
        # Plan 050 T6 — neovim as a declarative command-status SOURCE, proving the
        # shared-lib mechanism extends to a SECOND program with ZERO new plumbing:
        # only a new `events` map + a fold into this program's NATIVE hook installer.
        # For neovim that installer is an Overseer COMPONENT (not shell hooks like the
        # `shell` source, nor CC hooks) - a genuinely different event system, so it is
        # the strongest extensibility proof. Same shared lib, same single writer.
        tmuxCmdState = import ../../lib/tmux-cmd-state.nix { inherit pkgs lib; };
        ncfg = config.programs.neovim.tmuxStatus;
        # mkProgramSource maps each Overseer STATUS -> a call to the ONE shared
        # `tmux-cmd-state` writer, validating every mapped state against the lib's
        ...
        nvimStatusMap = lib.concatStringsSep "\n"
          (lib.mapAttrsToList (status: cmd: "  ${status} = ${builtins.toJSON cmd},")
            nvimSource.commands);
        # The last-mile adapter: an Overseer component whose on_status fires the mapped
        # writer for this task's pane. Placed on the runtimepath via nixvim extraFiles
        # (lua/overseer/component/<name>.lua is how Overseer resolves a named
        # component) and referenced from component_aliases.default below.
        nvimComponentLua = ''
          -- GENERATED by nixcfg (plan 050 T6). Overseer component: drive the tmux
          -- per-pane @cmd_state marker from this task's lifecycle status, via the
          -- shared `tmux-cmd-state` writer. The status->command map is baked from Nix
          -- (programs.neovim.tmuxStatus.events).
          local status_cmd = {
          ${nvimStatusMap}
          }
          return {
            desc = "nixcfg plan 050: emit tmux per-pane @cmd_state from Overseer task status",
            constructor = function()
              return {
                on_status = function(self, task, status)
                  local cmd = status_cmd[status]
                  -- No-op outside tmux (the writer is a no-op too, but skip the fork).
                  if cmd and vim.env.TMUX then
      ...
      in
      {
        ...
        # Plan 050 T6 (decision D9) — declare the event->state map in neovim's OWN
        # namespace (mirrors programs.claude-code.tmuxStatus). No cross-config reads;
        # no auto-publish into programs.tmux.commandStatus.sources (T5 established that
        # a write to another module's option breaks standalone composition).
        options.programs.neovim.tmuxStatus = {
          enable = lib.mkOption {
            type = lib.types.bool;
            default = true;
            description = ''
              Drive the tmux per-pane command-status marker (@cmd_state; see
              programs.tmux.commandStatus) from Neovim's Overseer task runner, so a
              tmux window running an Overseer build/test shows its state at a glance:
              running (amber) while a task runs, done (green) on success, error (red)
              on failure - done/error suppressed on the ACTIVE pane by the shared
              writer. Installs one Overseer component (folded into
              component_aliases.default). No-op outside tmux. NOTE: the generic
              `shell` source already marks a pane running for nvim's whole lifetime;
              this source adds the finer-grained per-task transitions on top.
            '';
          };
          events = lib.mkOption {
            ...
            description = ''
              Map of Overseer STATUS name -> canonical command-status state (one of
              programs.tmux.commandStatus.states: attention/error/running/done). Each
              entry makes the generated Overseer component call the shared
              `tmux-cmd-state` writer with the mapped state when a task reaches that
              status. Overriding this re-maps which task outcomes raise which marker;
              an unknown state is a build-time error naming the offending status.
            '';
          };
        };
        ...
        config.programs.nixvim = {
          ...
          # Plan 050 T6 — install the generated Overseer command-status component on
          # the runtimepath. Overseer resolves a named component by requiring
          # lua/overseer/component/<name>.lua; nixvim's extraFiles adds files to the
          # rtp, so this makes "tmux_cmd_state" available to component_aliases.default
          # (appended below). Gated on the source being enabled - zero-cost when off.
          extraFiles = lib.mkIf ncfg.enable {
            "lua/overseer/component/tmux_cmd_state.lua".text = nvimComponentLua;
          };
            ...
            overseer = {
              ...
              settings = {
                ...
                component_aliases = {
                  ...
                  # Plan 050 T6 — fold the tmux command-status source into Overseer's
                  # native hook installer (every default task carries this component,
                  # so its on_status drives the @cmd_state marker). Appended only when
                  # the source is enabled, so the component file it names is present.
                  ++ lib.optional ncfg.enable "tmux_cmd_state";
                };
              };
            };
