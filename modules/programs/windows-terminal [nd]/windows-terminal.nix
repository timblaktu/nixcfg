...
#
# Provides:
#   flake.modules.homeManager.windows-terminal - WSL Windows Terminal config
#
# Features:
#   - Non-destructive JSON merging using jq
#   - Font, color scheme, and keybinding configuration
#   - Preserves existing user customizations
#   - merge-terminal-settings command for manual updates
#
# Usage in host config:
#   imports = [ inputs.self.modules.homeManager.windows-terminal ];
#   windowsTerminal = {
#     enable = true;  # defaults to true when WSL is detected
#     font.face = "CaskaydiaMono NFM, Noto Color Emoji";
#   };
{ config, lib, inputs, ... }:
{
  flake.modules = {
    # === Home Manager Windows Terminal Module ===
    homeManager.windows-terminal = { config, lib, pkgs, ... }:
      with lib;
      let
      ...
      in
      {
        options.windowsTerminal = {
          ...
          font = {
            face = mkOption {
              type = types.str;
              default = "CaskaydiaMono NFM, Noto Color Emoji";
              description = "Font face for Windows Terminal (primary, emoji fallback)";
            };
          ...
          home.activation.mergeWindowsTerminalSettings = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
            if [[ -n "''${WSL_DISTRO:-}" ]]; then
              ...
              ${mergeSettingsScript}/bin/merge-terminal-settings || {
                echo "Failed to merge Windows Terminal settings"
