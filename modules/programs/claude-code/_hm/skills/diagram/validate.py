...
"""
# Vendored and adapted from Agents365-ai/drawio-skill (MIT) —
# scripts/validate.py. Local change: reads this skill's native `.drawio.svg`
# wrapper by decoding the HTML-entity-encoded `content="..."` mxfile before
# linting (see load_tree); upstream only handled raw `.drawio` XML.
import argparse
import html
import re
import sys
...
def load_tree(path):
    """Parse a .drawio file OR this skill's .drawio.svg wrapper into an ElementTree.

    A .drawio.svg carries the editable mxfile HTML-entity-encoded in the <svg>
    `content="..."` attribute; the SVG body is only rendered output. Lint the
    real source: if a content attr holding an <mxfile> is present, decode and
    parse that; otherwise treat the file as raw .drawio XML.
    """
    ...
    if m:
        ...
        if "<mxfile" in decoded or "<mxGraphModel" in decoded:
            return ET.ElementTree(ET.fromstring(decoded))
    if "<svg" in raw[:512] and "mxfile" not in raw:
        sys.exit(f"error: {path} is a plain SVG with no embedded mxfile "
                 f"(orphaned/view-only) — nothing to lint")
    return ET.ElementTree(ET.fromstring(raw))


def main():
    ap = argparse.ArgumentParser(description="Lint a .drawio or .drawio.svg file for structural errors.")
