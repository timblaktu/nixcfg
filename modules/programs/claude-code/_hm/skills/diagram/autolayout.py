...
# scripts/autolayout.py. Local change: the group palette is the diagram skill's
# own 6-colour palette inline (SKILL.md Section 4), dropping upstream's
# styles/built-in/default.json preset dependency, which this skill does not ship.
import argparse
import json
import shlex
import subprocess
...
# Group colours are this skill's own (fill, stroke) palette (SKILL.md Section 4:
# blue, green, orange, purple, yellow, red). When a grouped graph is laid out,
# each top-level group takes the next colour in order so related modules read as
# a coloured cluster. Nodes that carry their own `style` keep it; only styleless
# grouped nodes are tinted. Disable with --mono.
