...
### P9 — Package-health: fix `build-docling` (upstream FOD hash-drift) `TASK:COMPLETE 2026-08-23` (dep P6 — COMPLETE)
> **RESOLUTION (Tim, 2026-08-23):** COMPLETE for its filed scope — the nlohmann FOD hash-drift is fixed and
> the full docling C++ closure builds (CI-proven). The distinct facet-4 blocker (docling-parse won't compile
> against nlohmann under GCC14 with either 3.10.5 or 3.11.3) was SPLIT to deferred, non-gating **P10**, which
> also owns re-enabling build-docling in the nightly matrix. See "P9 OUTCOME (2026-08-23)" below.
...
### P11 — CI-as-code: generate CI from a flake-SSOT classification map `TASK:COMPLETE 2026-08-23` (dep P6 — COMPLETE)
...
#### P11 COMPLETE (2026-08-23) — all deliverables done + proven green in GitHub CI
**Interactive checkpoint resolved (Tim, 2026-08-23):** after two rounds of review Tim corrected the framing
(nixcfg-work is a DOWNSTREAM CONSUMER of nixcfg, not a fork; what crosses the flake-input edge is small: the
dispatch convention + the classification vocabulary + a reusable mechanism, NOT nixcfg's classification data or
pipeline). Decisions: (a) export the mechanism as reusable `ci.lib.{mkMatrix,mkDriftGuard}` so nixcfg-work
imports it and applies to ITS OWN checks (commit 5041c44); (b) systems policy = behavior-preserving x86-only for
eval/lint/build/qemu now, expand aarch64 later in the GitLab phase (nspawn stays dual-arch). Tim also directed:
in-tree docs must specify the desired END STATE conceptually — no jargon, no plan/task references, no impl
details (those belong in .claude), no hard-wrapping, no emdashes. `docs/CI-MODEL.md` rewritten accordingly
(commit 176f83b). **DoD (iii) PROVEN:** per-PR CI run 32668397134 = SUCCESS (62 jobs green, 0 fail, 3 nightly
skipped; the new `Generate CI matrix` prep job + `ci-matrix-sync` guard both green; every generated job mirrors
the prior suite). Nightly workflow_dispatch 32668955741 = prep job green + nightly jobs correctly un-gated and
expanded from the manifest (tarballs from `.#ci.tarballs`, pkgs from `nightly.build` with build-docling absent
via enabled=false, compose-stack from `nightly.qemu`); heavy ML/tarball builds are unchanged from the prior
green nightly. **Next:** P11 unblocks merging the PR to main and P8 (nixcfg-work GitLab CI).
...
### P7 triage (2026-08-23) — only `monitoring` HM smoke is worth doing now
Tim asked for a per-item assessment (low-hanging fruit / high value / easy) of the deferred Tier-B
backlog before merging PR #6. NONE of these are forks of nixcfg; they are integrations with external
tools/services (aptly/apt-cacher-ng, JFrog Artifactory, Bitwarden) or ride upstream flake inputs
(nix-darwin, NixOS-WSL). Verdicts:

- **monitoring (HM side)** — LOW-HANGING FRUIT, DO IT. Portable, no external deps. The `monitor-rebuild`
  / `monitor` / `monitor-tool` scripts + btop config + Tier-1 tool set had zero behavioral coverage
  (NixOS wrappers alone are asserted in `vm-wsl-dev-team-layers`). Added as `vm-monitoring` (nspawn HM
  test). ← the sole P7 item executed.
- **mss-clamp** — ALREADY COVERED (NixOS iptables mangle rule asserted in `vm-wsl-dev-team-layers`);
  only the Darwin pf/scrub variant is untested and that is blocked on real Mac hardware.
- **jfrog-cli** — CONDITIONAL, deferred. Only the SOPS-credential path is testable (mirror
  `vm-sops-secrets`); the rbw/Bitwarden path is interactive. Worth doing ONLY if the team actually
  uses jfrog-cli. Tim: "leave the jfrog for later."
- **nuc-apt-repo** — SKIP. High-effort integration (aptly service + REST publish + signed retrieval +
  apt-cacher-ng); host "was never really used" (its P1 "highest-value" call was already overridden in
  P2). Low value.
- **enterprise / wsl-enterprise / wsl-dev-team full layers** — DEFER. Cannot eval the full WSL stack in
  the test framework (needs `wsl.enable` + host-key deps); carrier layers already smoke-tested; eval-only
  coverage is sufficient. Poor ROI.
- **darwin hosts (powerbook, macbook-air)** — PLATFORM-BLOCKED. Real hosts, not samples, but Linux CI
  cannot run Darwin tests; eval-only is the ceiling here.
- **real rbw -> SSH-deploy** — SKIP. Interactive vault unlock + network + secret; violates
  "reproducible everywhere"; not CI-friendly without a fixture vault.
