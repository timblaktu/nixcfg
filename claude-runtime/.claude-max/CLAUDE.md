...
- **MANDATORY RESPONSE ENDING**: EVERY response MUST produce a continuation prompt for the next session after context is cleared. NO EXCEPTIONS. The continuation prompt must be **self-contained** - merge the task summary (what was done, commits, artifacts, what was NOT done) directly into the prompt. **Delivery**: Copy to clipboard via `cat <<'CONT' | clip.exe` (not inline in chat). Print a short confirmation like "Continuation prompt copied to clipboard (topic: <brief>)". For prompts exceeding ~4KB, write to `/tmp/continuation.md` and `claude-browse /tmp/continuation.md` instead. Nothing is lost because the prompt is on the clipboard or in a file - the user pastes it into the next session.
...
- **ALWAYS use relative paths for inter-document markdown links** in shared repos - works in GitLab UI and local editors alike. Never use full URIs for links to files within the same repo.
...
- **NEVER use emdashes** - Do NOT use the unicode emdash character (U+2014) anywhere: chat output, files, API content (MR descriptions, commit messages, PR bodies). Always use a single normal hyphen '-' instead. This applies to ALL output in ALL contexts.
- **Content delivery for copy-paste** - When producing content the user needs to copy-paste (commit messages, Slack messages, config snippets, etc.), do NOT put it in chat as a code-fenced block. Instead: (1) **clip.exe** for short content: `echo -n "content" | clip.exe` puts it directly on Windows clipboard; (2) **temp .md file + xdg-open** for long/formatted content: write to `/tmp/foo.md` then `xdg-open` it. This avoids indentation/wrapping artifacts from chat rendering. Does NOT apply when content is sent directly via API (e.g., `glab api -X PUT` for MR descriptions) since no copy-paste is needed.
