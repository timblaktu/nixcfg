# Shared Claude Code Configuration (All Accounts)

This file contains rules, preferences, and technical learnings shared across ALL Claude Code
accounts (PRO, MAX, work, etc.). Account-specific files reference this file and add only
account-specific overrides.

## Memory Placement Rule (CRITICAL)

**When adding or updating memory content, ALWAYS determine the correct location:**

- **This file** (`CLAUDE-SHARED.md`): Rules, preferences, technical learnings, workflow protocols,
  and any guidance that applies regardless of which account is active. **Default to shared** -
  most content belongs here. If in doubt, put it here.
- **Account-specific** (`CLAUDE.md` in `.claude-pro/`, `.claude-max/`, etc.): ONLY content that
  is truly specific to one account - e.g., account-specific model defaults, paths containing
  the account name, or temporary per-account overrides. This should be rare.

**Before writing memory**: Read this file first. Check if the content already exists or can be
merged with an existing section. Do not duplicate content between this file and account files.

**Shared file location**: `/home/tim/src/nixcfg/claude-runtime/.claude/CLAUDE-SHARED.md`
...
- `/nixmemory` (alias: `/usermemory`, `/globalmemory`) - Opens user-global memory file in editor
- `/nixremember <content>` (alias: `/userremember`, `/globalremember`) - Appends content to memory
- Built-in `/memory` and `#` commands will fail on read-only files - use /nix* versions instead
- These write to the account-specific CLAUDE.md (path varies by account - see account file)
...
- /refactor extract-function, rename-symbol
- /context cleanup, save, load

### Active Hooks
- Security checks, auto-formatting, linting, testing, git integration, logging, notifications: (configured per-project)
